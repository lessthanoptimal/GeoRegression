 ...
 * @author Peter Abeles
 */
public class MotionSe2PointSVD implements MotionTransformPoint<Se2,Point2D_F64> {

    Se2 motion = new Se2();

    @Override
    public Se2 getMotion() {
        return motion;
    }
    ...
    public boolean process( List<Point2D_F64> fromPts , List<Point2D_F64> toPts ) {
        ...
        Point2D_F64 meanFrom = UtilPoint2D.mean(fromPts);
        Point2D_F64 meanTo = UtilPoint2D.mean(toPts);
        ...
        for( int i = 0; i < N; i++ ) {
            ...
            s11 += f.x*t.x; s12 += f.x*t.y;
            s21 += f.y*t.x; s22 += f.y*t.y;
        }

        s11 = s11/N - m11; s12 = s12/N - m12;
        s21 = s21/N - m21; s22 = s22/N - m22;
        ...
        // save the results
        GeometryMath.rotate(yaw,meanFrom,meanFrom);
