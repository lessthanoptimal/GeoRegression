 ...
 * @author Peter Abeles
 */
public class MotionSe3PointCrossCovariance implements MotionTransformPoint<Se3,Point3D_F64> {

    // first 4 elements are quaternions and last 3 are translation
    private double param[] = new double[7];

    // rigid body motion
    private Se3 motion = new Se3();
    ...
    public boolean process( List<Point3D_F64> fromPts , List<Point3D_F64> toPts ) {
        ...
        Point3D_F64 meanFrom = UtilPoint3D.mean(fromPts);
        Point3D_F64 meanTo = UtilPoint3D.mean(toPts);
        ...
        for( int i = 0; i < N; i++ ) {
            ...
            s11 += f.x*t.x; s12 += f.x*t.y; s13 += f.x*t.z;
            s21 += f.y*t.x; s22 += f.y*t.y; s23 += f.y*t.z;
            s31 += f.z*t.x; s32 += f.z*t.y; s33 += f.z*t.z;
        }

        s11 = s11/N - m11; s12 = s12/N - m12; s13 = s13/N - m13;
        s21 = s21/N - m21; s22 = s22/N - m22; s23 = s23/N - m23;
        s31 = s31/N - m31; s32 = s32/N - m32; s33 = s33/N - m33;
...
//        Q.print();
        ...
        GeometryMath.mult(motion.getR(),meanFrom,meanFrom);
