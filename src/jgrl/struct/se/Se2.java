 ...
 * @author Peter Abeles
 */
public class Se2 implements SpecialEuclidean<Se2> {

    // the translational component
    private Vector2D_F64 tran = new Vector2D_F64();

    // rotational component parameterized for speed
    private double c; // cos(yaw)
    private double s; // sin(yaw)

    public Se2(GeoTuple2D_F64 tran, double yaw) {
        this( tran.getX() , tran.getY() , yaw );
    }

    public Se2( double x , double y , double yaw ) {
        set(x,y,yaw);
    }

    public Se2( double x , double y , double cosYaw , double sinYaw ) {
    ...
    @Override
    public Se2 createInstance() {
        return new Se2();
    }

    @Override
    public Se2 concat( Se2 second , Se2 result ) {
        if( result == null )
            result = new Se2();
    ...
    public Se2 copy() {
        return new Se2(tran.x,tran.y,c,s);
    }
