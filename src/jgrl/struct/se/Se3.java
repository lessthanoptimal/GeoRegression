 ...
 * @author Peter Abeles
 */
public class Se3 implements SpecialEuclidean<Se3> {
    // rotation matrix
     ...
     * @param T Translation.
     */
    public Se3( DenseMatrix64F R , Vector3D_F64 T ) {
        this(R,T,false);
    }
     ...
     */
    public Se3( DenseMatrix64F R , Vector3D_F64 T , boolean assign ) {
        if( assign ) {
            this.R = R;
            this.T = T;
        } else {
    ...
    @Override
    public Se3 createInstance() {
        return new Se3();
    }

    @Override
    public Se3 concat(Se3 second, Se3 result) {
        if( result == null )
            result = new Se3();

        CommonOps.mult(second.getR(),getR(),result.getR());
        GeometryMath.mult(second.getR(),getT(),result.getT());
        GeometryMath.add(second.getT(),result.getT(),result.getT());

        return result;
    }

    @Override
    public Se3 invert(Se3 inverse) {
        ...
        // -R^T*T
        GeometryMath.multTran(R,T,inverse.T);
        GeometryMath.changeSign(inverse.T);
