...
public class FitEllipseAlgebraic_F32 {
	...
	public boolean process( List<Point2D_F32> points ) {
		...
		ellipse.a = (float)a1.data[0];
		ellipse.b = (float)(a1.data[1]/2);
		ellipse.c = (float)a1.data[2];
		ellipse.d = (float)(Ta1.data[0]/2);
		ellipse.e = (float)(Ta1.data[1]/2);
		ellipse.f = (float)Ta1.data[2];
	...
	private DenseMatrix64F selectBestEigenVector() {
		...
		for( int i = 0; i < eigen.getNumberOfEigenvalues(); i++ ) {
			...
			// evaluate a'*C*a = 1
			float cond = (float)(4*v.get(0)*v.get(2) - v.get(1)*v.get(1));
