...
public class FitCurve_F64 {
	 ...
	 */
	public static QuadraticPolynomial2D_F64 fit(List<Point2D_F64> points , @Nullable QuadraticPolynomial2D_F64 output ) {
		if( output == null )
			output = new QuadraticPolynomial2D_F64();

		fit(points,output,new DMatrix3x3());

		return output;
	}

	/**
	 * Low level version of {@link #fit(List, QuadraticPolynomial2D_F64)} which allows internal work variables
	 * to be passed in
	 */
	public static void fit(List<Point2D_F64> points , QuadraticPolynomial2D_F64 output , DMatrix3x3 work ) {
		if( points.size() < 3 )
		...
		CommonOps_DDF3.invert(A,A);// TODO use a symmetric inverse. Should be slightly faster
