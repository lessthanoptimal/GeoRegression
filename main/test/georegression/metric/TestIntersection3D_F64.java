...
public class TestIntersection3D_F64 extends GeoRegressionJUnit {

	@Test void intersect_planenorm_linepara() {
		// simple case with a known solution
		var plane = new PlaneNormal3D_F64(2, 1, 0, 2, 0, 0);
		var line = new LineParametric3D_F64(0, 0, 0, 3, 0, 0);
	...
	@Test void intersect_planegen_linepara() {
		// simple case with a known solution
		var plane = new PlaneNormal3D_F64(2, 1, 0, 2, 0, 0);
		var line = new LineParametric3D_F64(0, 0, 0, 3, 0, 0);
	...
	@Test void intersect_plane_plane() {
		var a = new PlaneGeneral3D_F64(3, -4, 0.5, 6);
		var b = new PlaneGeneral3D_F64(1.5, 0.95, -4, -2);
	...
	@Test void intersection_triangle_ls() {
		var ls = new LineSegment3D_F64();
		var p = new Point3D_F64();

		// degenerate triangle
		var triangle = new Triangle3D_F64(1, 1, 1, 2, 2, 2, 3, 3, 3);
	...
	@Test void intersection_triangle_line() {
		var line = new LineParametric3D_F64();
		var p = new Point3D_F64();
	...
	@Test void intersect_poly_line() {
		// Simple test cases
		var polygon = new FastArray<>(Point3D_F64.class);
		...
		var line = new LineParametric3D_F64(-0.5, 0.5, 0, 0, 0, 1);
...
//		assertTrue(found.distance(-0.5,0.5,2) <= GrlConstants.TEST_F64);
	...
	@Test void contained_boxLength_point() {
		var box = new BoxLength3D_F64(2, 3, 4, 1, 1.5, 2.5);
	...
	@Test void contained_box_point() {
		var box = new Box3D_F64(2, 3, 4, 3, 4.5, 6.5);
	...
	@Test void contained2_box_point() {
		var box = new Box3D_F64(2, 3, 4, 3, 4.5, 6.5);
	...
	@Test void intersect_line_sphere() {
		var a = new Point3D_F64();
		var b = new Point3D_F64();
	 ...
	 */
	@Test void intersectConvex() {
		var polygon = new DogArray<>(Point3D_F64::new);
	...
	@Test void intersection1_ray_cyl() {
		var cylinder = new Cylinder3D_F64(1, 2, 3, 0, 0, 1, 2);
		var found = new Point3D_F64();

		// Check a few cases which can be easily hand computed
		var ray = new LineParametric3D_F64(10, 2, 0, -1, 0, 0);
		assertTrue(Intersection3D_F64.intersection1(ray, cylinder, found));
		assertTrue(found.isIdentical(3, 2, 0, UtilEjml.TEST_F64));

		ray.p.setTo(10, 0, 0); // exact intersection. This test might be unstable
		assertTrue(Intersection3D_F64.intersection1(ray, cylinder, found));
		assertTrue(found.isIdentical(1, 0, 0, UtilEjml.TEST_F64));

		ray.p.setTo(10, 1, 0); // two matches here
		assertTrue(Intersection3D_F64.intersection1(ray, cylinder, found));
		assertTrue(found.isIdentical(1 + Math.sqrt(2*2 - 1), 1, 0, UtilEjml.TEST_F64));

		// No Matches, outside the cylinder
		ray.p.setTo(10, 5, 0);
		assertFalse(Intersection3D_F64.intersection1(ray, cylinder, found));

		// point the ray in the other direction
		ray.p.setTo(10, 2, 0);
		ray.slope.x = 1;
		assertFalse(Intersection3D_F64.intersection1(ray, cylinder, found));

		// special case. ray's origin is inside the cylinder.
		ray.p.setTo(1, 2, 3);
		ray.slope.setTo(1, 0, 0);
		assertTrue(Intersection3D_F64.intersection1(ray, cylinder, found));
		assertTrue(found.isIdentical(3, 2, 3, UtilEjml.TEST_F64));

		// point the ray in the opposite direction
		ray.slope.setTo(-1, 0, 0);
		assertTrue(Intersection3D_F64.intersection1(ray, cylinder, found));
		assertTrue(found.isIdentical(-1, 2, 3, UtilEjml.TEST_F64));
	}
}
