...
public class TestUtilAngle {
	 ...
	 */
	@Test void bound() {
		assertEquals(PI*0.5, UtilAngle.bound(PI*2.5), 1e-5);
		assertEquals(-PI*0.5, UtilAngle.bound(-PI*2.5), 1e-5);
		assertEquals(PI*0.5, UtilAngle.bound(-PI*1.5), 1e-5);
		assertEquals(-PI*0.5, UtilAngle.bound(PI*1.5), 1e-5);
		assertEquals(PI*0.3, UtilAngle.bound(PI*8.3), 1e-5);
		assertEquals(PI*0.3, UtilAngle.bound(PI*0.3), 1e-5);
		assertEquals(-PI*0.9, UtilAngle.bound(-PI*0.9), 1e-5);
	}

	@Test void boundHalf_F64() {
	...
	@Test void minus() {
		// Check direction
		assertTrue(UtilAngle.minus(0.1, 0.2) < 0);
		assertTrue(UtilAngle.minus(0.2, 0.1) > 0);
		assertTrue(UtilAngle.minus(-0.1, -0.2) > 0);
		assertTrue(UtilAngle.minus(-0.2, -0.1) < 0);

		// Check some hand computed solutions
		assertEquals(-0.1, UtilAngle.minus(0.1, 0.2), 1e-5);
		assertEquals(0.1, UtilAngle.minus(0.2, 0.1), 1e-5);
		assertEquals(0.1, UtilAngle.minus(-0.1, -0.2), 1e-5);
		assertEquals(-0.1, UtilAngle.minus(-0.2, -0.1), 1e-5);

		// Check specific cases using definition
		checkMinus(PI, 0, 1e-5);
		checkMinus(-PI, 0, 1e-5);
		checkMinus(0, PI, 1e-5);
		checkMinus(0, 0, 1e-5);
		checkMinus(-PI, PI, 1e-5);
		checkMinus(PI, -PI, 1e-5);
		checkMinus(0.1*PI, -0.9*PI, 1e-5);
		checkMinus(-0.9*PI, 0.1*PI, 1e-5);
		checkMinus(PI, 0.5*PI, 1e-5);
		checkMinus(-PI, 0.5*PI, 1e-5);
		checkMinus(0.9*PI, -0.9*PI, 1e-5);
		checkMinus(-0.9*PI, 0.9*PI, 1e-5);
	}

	/**
	 * Checks minus operator using its definition.
	 *
	 * c = a - b
	 * a = c + b
	 */
	private static void checkMinus( double a, double b, double tol ) {
		double found = UtilAngle.minus(a, b);
		assertEquals(0.0, dist(a, UtilAngle.bound(b + found)), tol);
	}

	/**
	 * Test it with random inputs using the expected mathematical properties
	 */
	@Test void minus_rand() {
		for (int trial = 0; trial < 100; trial++) {
			double a = UtilAngle.bound(rand.nextDouble()*2.0*Math.PI);
			double b = UtilAngle.bound(rand.nextDouble()*2.0*Math.PI);

			double result = UtilAngle.minus(a, b);
			double check = UtilAngle.bound(b + result);
			assertEquals(a, check, TEST_F64);
		}
	}
	...
	@Test void reflectZeroToOne_F64() {
	...
	@Test void reflectZeroToOne_F32() {
