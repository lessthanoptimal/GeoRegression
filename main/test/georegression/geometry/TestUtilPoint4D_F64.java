...
public class TestUtilPoint4D_F64 extends GeoRegressionJUnit {
	...
	@Test void randomN_4D() {
		...
		double tol = 0.025;
		assertEquals(center.x,meanX,tol);
		assertEquals(center.y,meanY,tol);
		assertEquals(center.z,meanZ,tol);
		assertEquals(center.w,meanW,tol);

		assertEquals(0.5,stdX,tol);
		assertEquals(0.5,stdY,tol);
		assertEquals(0.5,stdZ,tol);
		assertEquals(0.5,stdW,tol);
	}

	@Test void randomN_3D() {
	...
	@Test void random_min_max() {
		...
		for (int i = 0; i < list.size(); i++) {
			Point4D_F64 p = list.get(i);
			for (int j = 0; j < 4; j++) {
				foundLower.set(j,Math.min(foundLower.get(j),p.get(j)));
				foundUpper.set(j,Math.max(foundUpper.get(j),p.get(j)));
			}
		}

		for (int i = 0; i < 4; i++) {
			assertTrue(foundLower.get(i)<lower+0.05);
			assertTrue(foundUpper.get(i)>upper-0.05);
		}
	}
