...
public class TestTangentLinesTwoEllipses_F32 {
	...
	@Test
	public void process() {
		TangentLinesTwoEllipses_F32 alg = new TangentLinesTwoEllipses_F32(GrlConstants.FLOAT_TEST_TOL, 20);
...
//			System.out.println(i+"  Theta = "+theta);
	...
	private void checkResults(EllipseRotated_F32 ellipseA, EllipseRotated_F32 ellipseB ,
		...
		// make sure all the points are unique
		assertFalse(tangentA0.distance(tangentA1) <= GrlConstants.FLOAT_TEST_TOL );
		assertFalse(tangentA0.distance(tangentA2) <= GrlConstants.FLOAT_TEST_TOL );
		assertFalse(tangentA0.distance(tangentA3) <= GrlConstants.FLOAT_TEST_TOL );
		assertFalse(tangentA1.distance(tangentA2) <= GrlConstants.FLOAT_TEST_TOL );
		assertFalse(tangentA1.distance(tangentA3) <= GrlConstants.FLOAT_TEST_TOL );
		assertFalse(tangentA2.distance(tangentA3) <= GrlConstants.FLOAT_TEST_TOL );

		assertFalse(tangentB0.distance(tangentB1) <= GrlConstants.FLOAT_TEST_TOL );

		if( completeTest ) {
			assertFalse(tangentB0.distance(tangentB2) <= GrlConstants.FLOAT_TEST_TOL );
			assertFalse(tangentB0.distance(tangentB3) <= GrlConstants.FLOAT_TEST_TOL );
			assertFalse(tangentB1.distance(tangentB2) <= GrlConstants.FLOAT_TEST_TOL );
			assertFalse(tangentB1.distance(tangentB3) <= GrlConstants.FLOAT_TEST_TOL );
			assertFalse(tangentB2.distance(tangentB3) <= GrlConstants.FLOAT_TEST_TOL );
								 ...
								 EllipseRotated_F32 ellipseA, EllipseRotated_F32 ellipseB)
	{
		...
		assertTrue(UtilVector2D_F32.identicalSign(slopeX,slopeY, slopeA.x, slopeA.y, GrlConstants.FLOAT_TEST_TOL_SQRT ));
		assertTrue(UtilVector2D_F32.identicalSign(slopeX,slopeY, slopeB.x, slopeB.y, GrlConstants.FLOAT_TEST_TOL_SQRT ));
	...
	public void selectTangent() {
		...
		assertEquals(10, found.y, GrlConstants.FLOAT_TEST_TOL);

		assertEquals(1.5f*1.5f, alg.sumDifference, GrlConstants.FLOAT_TEST_TOL );

		assertTrue(alg.selectTangent(a,srcA,ellipse,found, true));
		assertNotEquals(2, found.x, GrlConstants.FLOAT_TEST_TOL);
		assertNotEquals(10, found.y, GrlConstants.FLOAT_TEST_TOL);
	}
}
