...
public class TestConvertRotation3D_F64 extends GeoRegressionJUnit {
	@Test void rodrigues4ToMatrix() {
	...
	@Test void rodriguesToEuler() throws InvocationTargetException, IllegalAccessException {
	...
	@Test void rodriguesToQuaternion_3() {
		double rx = 0.4;
		double ry = 2;
		double rz = -0.5;
	...
	@Test void rodriguesToQuaternion() {
	...
	@Test void quaternionToRodrigues() {
	...
	@Test void quaternionToEuler() throws InvocationTargetException, IllegalAccessException {
	...
	// Synthetic test that sees how it handles the case where identity is not exactly identity.
	// This is similar to the real-world test cases below
	@Test void matrixToRodrigues_scaledIdentity() {
		DMatrixRMaj R = CommonOps_DDRM.identity(3);
		CommonOps_DDRM.scale(1.0 - UtilEjml.TESTP_F64, R);
		var rod = ConvertRotation3D_F64.matrixToRodrigues(R, null);
		assertEquals(0.0, rod.theta, UtilEjml.TESTP_F64);

		// Pin the axis in this ambiguous case
		assertEquals(1.0, rod.unitAxisRotation.x, 0);
		assertEquals(0.0, rod.unitAxisRotation.y, 0);
		assertEquals(0.0, rod.unitAxisRotation.z, 0);
	}
	...
	private void checkMatrixToRodrigues( Rodrigues_F64 rodInput ) {
		...
		assertEquals(1, Math.abs(dot), UtilEjml.TESTP_F64);
		...
		assertTrue(UtilAngle.dist(rodInput.theta*dot, found.theta) <= UtilEjml.TESTP_F64);
	}
	...
	private void checkMatrixToRodrigues( double eulerX, double eulerY, double eulerZ ) {
		...
		assertTrue(MatrixFeatures_DDRM.isIdentical(M, found, UtilEjml.TESTP_F64));
	}
	...
	private void checkMatrixToRodrigues( Rodrigues_F64 input,
		...
		// if the lines are parallel the dot product will be 1 or -1
		assertEquals(1, Math.abs(found.unitAxisRotation.dot(expected.unitAxisRotation)), UtilEjml.TESTP_F64);

		// if the rotation vector is in the opposite direction then the found angle will be
		// the negative of the input. both are equivalent
		assertEquals(expected.theta, found.theta, UtilEjml.TESTP_F64);
	}

	/**
	 * A found test case where it failed
	 */
	@Test void matrixToRodrigues_case0() {
		...
		assertEquals(0, found.getTheta(), UtilEjml.TESTP_F64);
	}

	@Test void matrixToRodrigues_case1() {
		DMatrixRMaj R = UtilEjml.parse_DDRM(
	...
	@Test void matrixToRodrigues_case2() {
		DMatrixRMaj R = UtilEjml.parse_DDRM(
		...
		assertFalse(found.unitAxisRotation.isNaN());
		assertEquals( Math.PI, found.getTheta(), UtilEjml.TESTP_F64);
	}

	@Test void matrixToRodrigues_case3() {
		var R = new DMatrixRMaj(3, 3, true,
				0.9999999999995872, 5.551115123125783E-17, 3.469446951953614E-17,
				5.551115123125783E-17, 0.9999999999995871, 5.551115123125783E-17,
				3.469446951953614E-17, 5.551115123125783E-17, 0.9999999999995873);

		Rodrigues_F64 found = ConvertRotation3D_F64.matrixToRodrigues(R, (Rodrigues_F64)null);
		assertFalse(found.unitAxisRotation.isNaN());
		assertEquals(0.0, found.getTheta(), UtilEjml.TESTP_F64);
	}
	...
	@Test void matrixToEuler() throws InvocationTargetException, IllegalAccessException {
	 ...
	 */
	@Test void approximateRotationMatrix_random() {
	 ...
	 */
	@Test void approximateRotationMatrix_nochange() {
	...
	@Test void eulerToMatrix() throws InvocationTargetException, IllegalAccessException {
	...
	@Test void eulerToQuaternion() throws InvocationTargetException, IllegalAccessException {
