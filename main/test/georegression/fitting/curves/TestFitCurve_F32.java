...
public class TestFitCurve_F32 {
	...
	public void quadratic_polynomial_p3() {
		...
		QuadraticPolynomial2D_F32 found = FitCurve_F32.fit(points,null);

		// it should fit all the points perfectly
		for( Point2D_F32 p : points ) {
			assertEquals(0,found.evaluate(p.x,p.y),4*GrlConstants.TEST_F32);
		}
		...
		found = FitCurve_F32.fit(points,null);
		...
		assertEquals(0,found.a2,GrlConstants.TEST_F32);
		// it should fit all the points perfectly
		for( Point2D_F32 p : points ) {
			assertEquals(0,found.evaluate(p.x,p.y),4*GrlConstants.TEST_F32);
		}
