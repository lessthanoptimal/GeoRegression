...
public class TestSePointOps {
    ...
    public void transform_2d_single() {
        Se2 tran = new Se2(-2,3,Math.PI);

        Point2D_F64 pt = new Point2D_F64(2,4);

        // see if it creates a new instance correctly
        Point2D_F64 found = SePointOps.transform(tran,pt,null);
        ...
        found.set(10,10);
        SePointOps.transform(tran,pt,found);
        assertEquals(-4,found.getX(),1e-8);
        assertEquals(-1,found.getY(),1e-8);

        // modify the original
        SePointOps.transform(tran,pt,pt);
    ...
    public void transformReverse_2d_single() {
        Se2 tran = new Se2(-2,3,Math.PI/2.0);

        Point2D_F64 pt = new Point2D_F64(2,4);

        // see if it creates a new instance correctly
        Point2D_F64 found = SePointOps.transformReverse(tran,pt,null);
        ...
        // now provide it an input to work off of
        found.set(10,10);
        SePointOps.transformReverse(tran,pt,found);
        assertEquals(1,found.getX(),1e-8);
        assertEquals(-4,found.getY(),1e-8);

        // modify the original
        SePointOps.transformReverse(tran,pt,pt);
    ...
    public void transform_2d_array() {
        ...
        int N = 12;
        SePointOps.transform(tran,pts,N);
        for( int i = 0; i < N; i++ ) {
    ...
    public void transformReverse_3d_single() {
        ...
        SePointOps.transformReverse(se,P,Pt);
